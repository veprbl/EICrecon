...
std::unique_ptr<edm4hep::RawCalorimeterHitCollection> CalorimeterHitDigi::process(const edm4hep::SimCalorimeterHitCollection &simhits)  {
    ...
    for (const auto &[id, ixs] : merge_map) {
        ...
        if (m_cfg.readoutType == CalorimeterHitDigiConfig::kSimpleReadout) {
            ...
            const long long int n_max_photons = m_cfg.dyRangeADC * m_cfg.lightYield * m_cfg.photonDetectionEfficiency;
            const long long int n_max_pixels_fired = m_cfg.numEffectiveSipmPixels * (1 - exp(- n_max_photons / (double)m_cfg.numEffectiveSipmPixels));
            m_log->trace("n_photons_detected {}, n_pixels_fired {}, n_max_pixels_fired {}", n_photons_detected, n_pixels_fired, n_max_pixels_fired);
            adc = std::llround(ped + n_pixels_fired * (m_cfg.corrMeanScale + eResRel) / n_max_pixels_fired * m_cfg.capADC);
        }
